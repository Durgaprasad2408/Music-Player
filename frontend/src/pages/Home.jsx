...
const Home = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [playlistsRes, albumsRes, artistsRes, genresRes, moodsRes] = await Promise.all([
        apiClient.get('/playlists/featured'),
        apiClient.get('/albums?limit=6'),
        apiClient.get('/artists?limit=8'),
      ...
      if (user) {
        const recentRes = await apiClient.get('/playHistory/recent?limit=8');
        setRecentlyPlayed(recentRes.data || []);
      } else {
        const tracksRes = await apiClient.get('/tracks?limit=8');
        setRecentlyPlayed(tracksRes.data || []);
      }
    } catch (error) {
