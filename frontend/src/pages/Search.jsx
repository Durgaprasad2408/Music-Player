...
const Search = () => {
  ...
  const performSearch = async () => {
    ...
    try {
      // Search tracks
      const trackResponse = await apiClient.get(`/tracks/search?q=${encodeURIComponent(searchTerm)}&limit=12`);
      const trackData = trackResponse.data;

      // Search albums
      const albumResponse = await apiClient.get(`/albums/search?q=${encodeURIComponent(searchTerm)}&limit=6`);
      const albumData = albumResponse.data;

      // Search playlists - only show featured playlists and user's own playlists
      const playlistResponse = await apiClient.get(`/playlists/search?q=${encodeURIComponent(searchTerm)}&limit=6&featured=true&userId=${user?.id}`);
